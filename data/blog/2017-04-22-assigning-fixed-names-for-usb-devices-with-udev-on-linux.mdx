...
<p>
  Whilst you can use{' '}
  <a href="https://www.linux.com/news/udev-introduction-device-management-modern-linux-system">
    udev rules
  </a>{' '}
  with any USB device, I am writing this post to help anyone else who is also having issues with
  their USB printer name.
</p>
<p>
...
<p>
  ...
  <a href="https://askubuntu.com/questions/778367/cannot-print-from-canon-lbp6020-ubuntu-16-04">
    AskUbuntu
  </a>
  ), so instead I decided to create my own udev rule to assign a symlink from &#8216;lp1&#8217; to
  &#8216;lp0&#8217;.
</p>
...
<p>
  I stored this file within <code class="language-markup">/etc/udev/rules.d/</code> as{' '}
  <code class="language-markup">99-printer.rules</code>
</p>
<p>To find out your devices attributes you can run these commands:</p>
<ul>
  <li>
    idVendor:{' '}
    <code class="language-markup">udevadm info -a -n /dev/usb/lp0 | grep '{idVendor}'</code>
  </li>
  <li>
    idProduct:{' '}
    <code class="language-markup">udevadm info -a -n /dev/usb/lp0 | grep '{idProduct}'</code>
  </li>
  <li>
    serial: <code class="language-markup">udevadm info -a -n /dev/usb/lp0 | grep '{serial}'</code>
  </li>
</ul>
<p>
  ...
  <br />
  <code class="language-markup">sudo nano /etc/udev/rules.d/99-printer.rules</code>
</p>
<p>
...
<p>
  ...
  <br />
  <code class="language-markup">sudo ls -l /dev/usb</code>
</p>
<p>

...
<p>I stored this file within `/etc/udev/rules.d/` as `99-printer.rules`</p>
<p>To find out your devices attributes you can run these commands:</p>
<ul>
  <li>idVendor: `udevadm info -a -n /dev/usb/lp0 | grep '{idVendor}'`</li>
  <li>idProduct: `udevadm info -a -n /dev/usb/lp0 | grep '{idProduct}'`</li>
  <li>serial: `udevadm info -a -n /dev/usb/lp0 | grep '{serial}'`</li>
</ul>
<p>

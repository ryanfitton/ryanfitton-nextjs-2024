\-\-- layout: PostLayout title: \"WordPress will make PHP 7.0 the minimum required version, upgrade your PHP version soon\" authors: \[\'Ryan Fitton\'\] date: \'2019-01-17\' tags: \[\'Development\'\] draft: false summary: \'This has just come to my attention today, the developer team at WordPress will be making changes to the minimum PHP versions required by the WordPress core in the upcoming months. This is tracked in the core discussion here, a news article from ZDNet also talks about this change here. Starting with WordPress 5.1, a \[&hellip;\]\' \-\--
...
1.  Once you're logged into your cPanel account, you should have a 'MultiPHP Manager' option:\
    [![](/static/img/blog/2019/01/upgrade-php-5.6-1-300x112.png){loading="lazy" width="300" height="112" srcset="/static/img/blog/2019/01/upgrade-php-5.6-1-300x112.png 300w, /static/img/blog/2019/01/upgrade-php-5.6-1.png 483w" sizes="(max-width: 300px) 100vw, 300px"}](/static/img/blog/2019/01/upgrade-php-5.6-1.png)
2.   Select this option, and then select the domains for the version you wish to change:\
    [![](/static/img/blog/2019/01/upgrade-php-5.6-2-300x39.png){loading="lazy" width="300" height="39" srcset="/static/img/blog/2019/01/upgrade-php-5.6-2-300x39.png 300w, /static/img/blog/2019/01/upgrade-php-5.6-2.png 748w" sizes="(max-width: 300px) 100vw, 300px"}](/static/img/blog/2019/01/upgrade-php-5.6-2.png)
3.   Now select the new PHP version from the dropdown menu, and click 'Apply':\
    [![](/static/img/blog/2019/01/upgrade-php-5.6-3-300x101.png){loading="lazy" width="300" height="101" srcset="/static/img/blog/2019/01/upgrade-php-5.6-3-300x101.png 300w, /static/img/blog/2019/01/upgrade-php-5.6-3.png 737w" sizes="(max-width: 300px) 100vw, 300px"}](/static/img/blog/2019/01/upgrade-php-5.6-3.png)
...
10. Now edit your PHP.ini file, I do this to adjust the '`post_max_size`{.language-markup}' and '`upload_max_filesize`{.language-markup}' values as the defaults are too low. You PHP.ini may be kept in a different location, so be sure to change this:
...
12. One last step, option if you're not using phpMyAdmin 4.6.6. There is currently a compatibility between phpMyAdmin 4.6.6 and PHP 7.2. As described in [this StackOverflow post](https://stackoverflow.com/questions/48001569/phpmyadmin-count-parameter-must-be-an-array-or-an-object-that-implements-co).\
    ...
    And replace: `if ($options != null && count($options) > 0) {`{.language-markup}\
    \
    With: `if ($options != null && count((array)$options) > 0) {`{.language-markup}\
    \

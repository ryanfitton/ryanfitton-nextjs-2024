\-\-- layout: PostLayout title: \"Increasing the default ZFS Loop Device value in LXD on Ubuntu\" authors: \[\'Ryan Fitton\'\] date: \'2018-09-01\' tags: \[\'Code Snippets\'\] draft: false summary: \'I&#8217;ve recently installed LXD 2.x on my Ubuntu server but forgot to set a custom value for the loop device. The default value was 20GB, but I wanted this to be 1.4TB. It took a while of searching accross the web to find an easy solution to increase this size, as I found it tricky \[&hellip;\]\' \-\--
...
-   `POOL-NAME`{.language-markup}Your ZFS pool name, the default supplied by LXD is 'lxd'
...
You can also, check if the settings have been applied by running the `sudo zfs list`{.language-markup} command, you will see a result similar to below; make note of the values in the 'AVAIL' column as this should show your new pool size.

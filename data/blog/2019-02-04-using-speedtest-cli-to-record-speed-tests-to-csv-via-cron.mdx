\-\-- layout: PostLayout title: \"Using Speedtest-CLI to record speed tests to CSV via CRON\" authors: \[\'Ryan Fitton\'\] date: \'2019-02-04\' tags: \[\'Code Snippets\'\] draft: false summary: \'One of the most common speed testing services for Internet connections is Ookla&#8217;s Speedtest, and more often than not your ISP will either throttle your connection, or their network will slow down at peak times. Because of this, I wanted to make sure I am receiving the full connection speed which I am paying for. \[&hellip;\]\' \-\--
...
3.  Download the `speedtest-csv`{.language-markup} script. You will also find a [copy](https://github.com/ryanfitton/speedtest-cli-as-csv-cron/blob/master/speedtest-csv) of this script within the repo which you can, or re-download from the original author and make the changes to the file mentioned in step 5.
...
5.  You will need to edit a line in the file to so that it can be run from CRON:
    ...
    Find the line `cmd="speedtest-cli $opts"`{.language-markup} and add `/usr/local/bin/`{.language-markup} before it so it reads:
...
6.  Next move `speedtest-csv`{.language-markup} to `/usr/bin/`{.language-markup}:

    ``` language-markup
    sudo mv speedtest-csv /usr/bin/
    ```

7.  Create a folder for where your .csv file will be saved, I am saving mine within `/cron/speedtest/`{.language-markup}, but change this for whatever you'd like:

...
In your `functions.php`{.language-html} file, add the code shown below. This will load the required jQuery files, the Lazy library and setup a new instance of LazyLoad:
...
Also, within your `functions.php`{.language-html} add these functions which are used to add the required tags for the Image and Background image tags.
